...
- iOS privacy decision: `UIFileSharingEnabled` is intentionally unset in `ios/Runner/Info.plist`, and `LSSupportsOpeningDocumentsInPlace` is not enabled. The local database and model storage are excluded from OS backups, so Finder/iTunes file sharing stays disabled unless product requirements explicitly call for user-visible file export.
