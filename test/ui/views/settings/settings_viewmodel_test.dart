...
void main() {
  ...
  setUp(() {
    ...
    ).thenReturn(models.where((m) => m.type == AppModelType.inference).toList());
    when(
      () => modelService.downloadedEmbeddingModels,
    ).thenReturn(models.where((m) => m.type == AppModelType.embedding).toList());
    when(() => modelService.activeInferenceModel).thenReturn(models.first);
    when(() => modelService.activeEmbeddingModel).thenReturn(models[2]);
    when(() => modelService.modelStatusStream).thenAnswer((_) => statusController.stream);
    when(modelService.initialize).thenAnswer((_) async {});
    when(() => modelService.downloadModel(any())).thenAnswer((_) async {});
    when(() => modelService.switchInferenceModel(any())).thenAnswer((_) async {});
    when(() => modelService.switchEmbeddingModel(any())).thenAnswer((_) async {});
    when(() => ragSettings.maxTokens).thenReturn(null);
    when(() => ragSettings.setQueryExpansionEnabled(value: any(named: 'value'))).thenAnswer((_) async {});
    when(() => ragSettings.setRerankingEnabled(value: any(named: 'value'))).thenAnswer((_) async {});
    when(() => ragSettings.setContextualRetrievalEnabled(value: any(named: 'value'))).thenAnswer((_) async {});
    when(() => ragSettings.setChunkOverlapPercent(any())).thenAnswer((_) async {});
    when(() => ragSettings.setSemanticWeight(any())).thenAnswer((_) async {});
    when(() => ragSettings.setSearchTopK(any())).thenAnswer((_) async {});
    when(() => ragSettings.setMaxHistoryMessages(any())).thenAnswer((_) async {});
  ...
  test('setup listens for model status updates and notifies listeners', () async {
    final viewModel = SettingsViewModel(
      modelService: modelService,
      ragSettings: ragSettings,
      navigationService: navigationService,
  ...
  test('toggle and slider handlers persist new values', () async {
    ...
    viewModel.onChunkOverlapChanged(22);
    viewModel.onSemanticWeightChanged(0.4);
    viewModel.onSearchTopKChanged(4);
    viewModel.onMaxHistoryMessagesChanged(3);
    viewModel.onMaxTokensChanged(2048);
    ...
    await viewModel.onMaxTokensChangeEnd(viewModel.modelDefaultMaxTokens.toDouble());
    ...
    verify(() => ragSettings.setContextualRetrievalEnabled(value: true)).called(1);
  ...
  test('reads default dependencies from locator and preserves pending values during stale async updates', () async {
    final completer = Completer<void>();
    when(() => ragSettings.setChunkOverlapPercent(any())).thenAnswer((_) => completer.future);
